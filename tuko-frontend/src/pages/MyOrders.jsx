...
import { fetchOrders, loginAs } from "../services/api";
...
export default function MyOrders(){
 ...
 const [needsLogin,setNeedsLogin]=useState(false),[username,setUsername]=useState(""),[password,setPassword]=useState("");
 ...
 async function load(){
  try{
   ...
   if(e.response?.status===401){
    setNeedsLogin(true);
    setMessage("Sign in with your customer account to see orders.");
   }else{
 ...
 async function signIn(e){
  ...
  setMessage("");
  try{
   await loginAs("customer",username,password);
   await load();
  }catch(e){
   setNeedsLogin(true);
   setMessage(e.response?.data?.detail||"Customer login failed.");
  }
 }
 ...
 return <div className="min-h-screen bg-[#fffaf3] p-4 text-slate-900 dark:bg-slate-950 dark:text-white"><div className="mx-auto max-w-5xl">
  ...
  {needsLogin&&<form onSubmit={signIn} className="mx-auto mt-6 max-w-md rounded-[2rem] bg-white p-6 shadow-xl dark:bg-slate-900">
    <p className="text-sm font-bold text-emerald-600">Customer access</p>
    <h2 className="mt-1 text-3xl font-black">Track your orders</h2>
    <input value={username} onChange={e=>setUsername(e.target.value)} placeholder="Customer username" className="mt-5 w-full rounded-2xl border p-4 dark:border-slate-700 dark:bg-slate-950"/>
    <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Password" className="mt-3 w-full rounded-2xl border p-4 dark:border-slate-700 dark:bg-slate-950"/>
    <button className="mt-4 w-full rounded-2xl bg-emerald-600 py-4 font-black text-white">Sign in & view orders</button>
  </form>}

  {!needsLogin&&<div className="mt-8 space-y-6">{orders.map(o=>{const current=stages.indexOf(o.status);return <article key={o.id} className="overflow-hidden rounded-[2rem] bg-white shadow-sm dark:bg-slate-900">
